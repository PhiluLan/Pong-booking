...
export default function BookingPage(){
  ...
  const total=useMemo(()=>time?basePrice(time,hours,tables,config)+extrasPrice:0,[time,hours,tables,extrasPrice,config]);
    ...
    <section className="simple-shell">
      ...
      <div className="simple-card">
        ...
        {step===1&&<section className="flow-step"><p className="flow-kicker">1 von 4 · Zeit wählen</p><h2>Wann passt es euch?</h2>
          ...
          <div className="sticky-next"><div>{time?<><small>{dateLabel(date)} · {time}–{endTime}</small><strong>ab {money(basePrice(time,hours,1,config))}</strong></>:<span>Wähle eine Startzeit</span>}</div><button disabled={!time} onClick={()=>setStep(2)}>Weiter</button></div>
        </section>}
        {step===2&&<section className="flow-step"><button className="flow-back" onClick={()=>setStep(1)}>← Zeit ändern</button><p className="flow-kicker">2 von 4 · Tische wählen</p><h2>Wie viele Tische?</h2><p className="flow-copy">Wir weisen euch automatisch die nächsten freien Tische zu.</p><div className="table-options">{Array.from({length:available},(_,i)=>i+1).map(n=><button key={n} className={tables===n?"selected":""} onClick={()=>setTables(n)}><span className="table-number">{n}</span><span><strong>{n===1?"1 Tisch":`${n} Tische`}</strong><small>für bis zu {n*8} Personen</small></span><b>{money(basePrice(time,hours,n,config))}</b></button>)}</div><div className="friendly-note"><strong>Keine Tischnummer nötig.</strong><span>Deine Tische werden beim Abschluss automatisch reserviert.</span></div><div className="sticky-next"><div><small>{dateLabel(date)} · {time}–{endTime}</small><strong>{tables} {tables===1?"Tisch":"Tische"} · {money(basePrice(time,hours,tables,config))}</strong></div><button onClick={()=>setStep(3)}>Weiter</button></div></section>}
